...
// TEAM_035: Email sending is now handled by emailService with automatic Web3Forms fallback
import { sendBookingEmails, EmailProvider } from '../emailService';

// Earliest bookable date: May 28, 2026
const MIN_BOOKING_DATE = new Date(2026, 4, 28); // month is 0-indexed, so 4 = May
...
const BookingSection: React.FC<{ initialServiceId?: string | null }> = ({ initialServiceId }) => {
  ...
  // TEAM_035: Track which email provider was used for conditional success messaging
  ...
  const [selectedDate, setSelectedDate] = useState<string>(() => {
    ...
    const defaultDate = today > MIN_BOOKING_DATE ? today : MIN_BOOKING_DATE;
    return defaultDate.toISOString().split('T')[0];
  });
  const [selectedTime, setSelectedTime] = useState<string>('');
  const [currentMonth, setCurrentMonth] = useState(() => {
    const today = new Date();
    return today > MIN_BOOKING_DATE ? today : new Date(MIN_BOOKING_DATE);
  });
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      // TEAM_035: Send both admin + customer emails (auto-fallback to Web3Forms if EmailJS limit reached)
      const provider = await sendBookingEmails(templateParams);
              ...
              <div>
                ...
                <div className="space-y-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="pt-1">
                      {/* TEAM_034: Warm terracotta for selection tag */}
                      <h3 className="text-[10px] font-bold text-terracotta-600 uppercase tracking-widest mb-2">{t.booking.selected}</h3>
                  ...
                  {/* TEAM_034: Terracotta border for description */}
                  <p className="text-stone-500 text-sm font-light leading-relaxed border-l-2 border-terracotta-300 pl-4 py-1">
                  ...
                  <div className="flex gap-4 pt-4">
                    {/* TEAM_034: Peace slate blue for details icons */}
                  ...
                  {bookingStep === 0 ? (
                    <>
                      ...
                      <div className="flex items-center gap-4">
                        <button
                          disabled={currentMonth.getFullYear() === MIN_BOOKING_DATE.getFullYear() && currentMonth.getMonth() === MIN_BOOKING_DATE.getMonth()}
                  ...
                  <div className="animate-fade-in">
                    ...
                    <div className="grid grid-cols-7 gap-1 mb-8">
                      ...
                      {getDaysInMonth(currentMonth).map((day, idx) => {
                        ...
                        const isPast = day < MIN_BOOKING_DATE;
                        ...
                        return (
                          <button
                            ...
                            className={`
                                ...
                                ${isSelected ? 'bg-pink-500 text-white shadow-lg' : isPast ? 'text-stone-300 cursor-not-allowed opacity-50' : 'hover:bg-pink-50 text-stone-800'}
                                ${isToday && !isSelected ? 'text-terracotta-600 ring-1 ring-terracotta-100/50' : ''}
                              `}
                          >
                            {day.getDate()}
                          </button>
                        ...
                        {timeSlots.length > 0 ? timeSlots.map(t => (
                          <button
                            ...
                            className={`
                                ...
                                ${selectedTime === t ? 'bg-slate-500 text-white shadow-lg shadow-slate-100/50' : 'bg-stone-50 text-stone-600 hover:bg-stone-100'}
                              `}
                          >
                            {t}
                          </button>
                ...
                <div className="mt-auto pt-8 border-t border-stone-50 flex items-center justify-between">
                  ...
                  {bookingStep === 0 ? (
                    ...
                    <button
                      ...
                      className="px-8 py-4 bg-black text-white rounded-2xl text-[10px] uppercase tracking-widest font-bold disabled:opacity-20 hover:bg-terracotta-500 hover:text-white transition-all flex items-center gap-3"
                    >
